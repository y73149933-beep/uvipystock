...
async def seed_database(session: AsyncSession) -> None:
    ...
    # Give test users starting balances (USD + BTC only)
    await _ensure_balance(session, test_user.id, "USD", Decimal("100000"))
    await _ensure_balance(session, test_user.id, "BTC", Decimal("10"))
    await _ensure_balance(session, test2_user.id, "USD", Decimal("100000"))
    await _ensure_balance(session, test2_user.id, "BTC", Decimal("10"))

    # Only one default pair — admin adds more via Market page
    pairs_data = [
        ("BTC/USD", "BTC", "USD", 2, 8),
    ]
