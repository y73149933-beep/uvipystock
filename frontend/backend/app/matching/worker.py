...
class MatchingWorker:
    ...
    async def _handle_place(self, payload: OrderActionPayload) -> dict[str, Any]:
        ...
        elif payload.type == "market" and payload.side == "buy":
            # Market buy: ALWAYS refund unused locked quote asset.
            # This runs even when remaining=0 (fully filled) because the
            # worst-case lock (best_ask * 1.01 * qty) is always >= actual cost.
    ...
    async def _refund_market_buy_leftover(
        ...
        """Refund unused locked quote asset after a market buy.

        Market buy locks worst-case = best_ask * 1.01 * qty at placement.
        After matching, the actual cost may be less. This method:
          1. Loads the user's quote balance (FOR UPDATE)
          2. Unlocks ALL remaining locked_balance for the quote asset
             back to available. The settle operation already deducted
             the actual trade cost from locked, so whatever remains
             in locked is the unused buffer.
        """
        from app.db.session import async_session_factory
